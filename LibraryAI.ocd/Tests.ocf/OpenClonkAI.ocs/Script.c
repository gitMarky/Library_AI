...
global func CreateEnemy(id clonktype, int x, int y, int plr, array contents, int life)
{
	...
	AI_OpenClonk->AddAI(enemy);
	AI_OpenClonk->SetMaxAggroDistance(enemy, LandscapeWidth());
	AI_OpenClonk->SetGuardRange(enemy, 0, 0, LandscapeWidth(), LandscapeHeight());
	enemy->AddEnergyBar();
	return enemy;
}
...
global func Test10_OnStart(int plr)
{
	...
	AI_OpenClonk->SetAutoSearchTarget(clubber, false);
